...
| DOC-028 | IHP-IX | Subir publicaciones exige rol **editor/admin** en una organización (`create_unowned_dataset=false`), pero las solicitudes de membresía dan rol `member`. Decidir con UNESCO si las solicitudes desde IHP-IX otorgan `editor` por defecto o si se crea una organización "IHP-IX contributors". **Verificado en dev 2026-09-25 (como sysadmin)**: fluent acepta `{'en': …}`, `package_create` con el grupo del Member State funciona, el recurso se sube (url_type `upload`, formato PDF) y `contact_email` es obligatorio (el modal lo pide prellenado). Falta repetir con un editor no sysadmin y confirmar que el blob acaba en Azure. | Alta |
...
| DOC-030 | IHP-IX/Learning | En dev el plugin `learning` (catálogo nativo, rama `feat/learning-catalog`, ya fusionada en `dev210`) redirige `/courses` y `/ckan-admin/open-learning` al catálogo Learning y migró los cursos legacy a packages `type:learning`. El tipo de adjunto `course` y `ihpix_course_propose` siguen leyendo/escribiendo la tabla legacy `open_learning_course` (2 aprobados en dev), así que una propuesta nueva no aparecería en la cola que ven los sysadmins. Adaptar `ihpix_link_search kind=course` a `package_search fq=type:learning` y la propuesta al flujo de `ckanext-learning` cuando el plugin esté cargado. | Alta |
