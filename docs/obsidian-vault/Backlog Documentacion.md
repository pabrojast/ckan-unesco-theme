...
| DOC-028 | IHP-IX | Subir publicaciones exige rol **editor/admin** en una organización (`create_unowned_dataset=false`), pero las solicitudes de membresía dan rol `member`. Decidir con UNESCO si las solicitudes desde IHP-IX otorgan `editor` por defecto o si se crea una organización "IHP-IX contributors". Pendiente por confirmar en dev con datos reales: que `schemingdcat_fluent_text` acepte solo `{'en': …}`, que `package_create` con `groups` (Member State) no niegue el alta a usuarios que no son miembros del grupo (hay reintento sin grupos + aviso) y la subida server-side a Azure vía ckanext-cloudstorage. | Alta |
